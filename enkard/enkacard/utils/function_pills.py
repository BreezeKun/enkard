...
async def imagSize(link = "", image = None, fixed_width = 0, size = None):
    if not image:
        if link == "https://enka.network/ui/UI_Gacha_AvatarImg_PlayerGirl.png":
            imgs = open_file.PlayerGirl.copy()
        else:
    ...
    # Downloaded/asset images aren't guaranteed to already be RGBA (some source
    # PNGs are RGB or palette mode), and Pillow's alpha_composite requires an
    # exact mode match between the two images being combined.
...
async def centrText(text, witshRam = 100, razmer = 24, start = 0, Yram = 20, y = None, aling = "centry"):
    Text = ImageFont.truetype(open_file.font, razmer)
    maxDlina = witshRam
    while True:
        Text = ImageFont.truetype(open_file.font, razmer)
